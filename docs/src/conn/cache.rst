...
``Cache`` provides a way to supply a prexisting profile and copy that profile into a temp working directory.
When cache is used any changes made in LibreOffice will only last for the session.
Once the session is terminalted the working dir is invalidated.

Subsquent sessions will get a fresh profile copied from cache.

ooodev.adapter.text package
===========================

Subpackages
-----------

.. toctree::
   :maxdepth: 4

   ooodev.adapter.text.textfield

Submodules
----------

.. toctree::
   :maxdepth: 4

   ooodev.adapter.text.cell_properties_comp
   ooodev.adapter.text.cell_properties_partial_props
   ooodev.adapter.text.cell_range_comp
   ooodev.adapter.text.cell_range_partial_props
   ooodev.adapter.text.generic_text_document_comp
   ooodev.adapter.text.graphic_crop_struct_comp
   ooodev.adapter.text.numbering_rules_comp
   ooodev.adapter.text.numbering_style_comp
   ooodev.adapter.text.page_cursor_partial
   ooodev.adapter.text.paragraph_comp
   ooodev.adapter.text.paragraph_cursor_partial
   ooodev.adapter.text.relative_text_content_insert_partial
   ooodev.adapter.text.sentence_cursor_partial
   ooodev.adapter.text.simple_text_partial
   ooodev.adapter.text.table_column_separator_struct_comp
   ooodev.adapter.text.table_columns_comp
   ooodev.adapter.text.table_rows_comp
   ooodev.adapter.text.text_columns_comp
   ooodev.adapter.text.text_columns_partial
   ooodev.adapter.text.text_comp
   ooodev.adapter.text.text_content_comp
   ooodev.adapter.text.text_content_partial
   ooodev.adapter.text.text_cursor_comp
   ooodev.adapter.text.text_cursor_partial
   ooodev.adapter.text.text_document_comp
   ooodev.adapter.text.text_field_comp
   ooodev.adapter.text.text_frame_comp
   ooodev.adapter.text.text_frames
   ooodev.adapter.text.text_partial
   ooodev.adapter.text.text_portion_comp
   ooodev.adapter.text.text_portion_type_kind
   ooodev.adapter.text.text_range_comp
   ooodev.adapter.text.text_range_partial
   ooodev.adapter.text.text_section_comp
   ooodev.adapter.text.text_section_partial
   ooodev.adapter.text.text_table_comp
   ooodev.adapter.text.text_table_partial
   ooodev.adapter.text.text_table_properties_partial
   ooodev.adapter.text.text_table_row_comp
   ooodev.adapter.text.text_tables_comp
   ooodev.adapter.text.text_view_cursor_comp
   ooodev.adapter.text.text_view_cursor_partial
   ooodev.adapter.text.web_document_comp
   ooodev.adapter.text.word_cursor_partial

Module contents
---------------

.. automodule:: ooodev.adapter.text
   :members:
   :undoc-members:
   :show-inheritance:

...
class ViewState:
    ...
    def report(self) -> None:
        ...
        print(
            f"  Cursor pos (column, row): ({self._cursor_column}, {self._cursor_row}) or '{mCalc.Calc.get_cell_str(col=self._cursor_column, row=self._cursor_row)}'"
        )
        if self._col_split_mode == 1 and self._row_split_mode == 1:
            print(
                f"  Sheet is split vertically and horizontally at {self._vertical_split} / {self.get_horizontal_split}"
            )

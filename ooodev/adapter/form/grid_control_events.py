...
def on_lazy_cb(source: Any, event: ListenerEventArgs) -> None:
    ...
    Args:
        source (Any): Expected to be an instance of GridControlEvents that is a partial class of a component based class.
    ...
    if not isinstance(source, GridControlEvents):
        return
    if not hasattr(source, "component"):
        return

    comp = cast("XGridControl", source.component)  # type: ignore
    comp.addGridControlListener(source.events_listener_grid_control)

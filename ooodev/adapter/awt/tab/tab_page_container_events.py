...
def on_lazy_cb(source: Any, event: ListenerEventArgs) -> None:
    ...
    Args:
        source (Any): Expected to be an instance of TabPageContainerEvents that is a partial class of a component based class.
    ...
    if not isinstance(source, TabPageContainerEvents):
        return
    if not hasattr(source, "component"):
        return

    comp = cast("XTabPageContainer", source.component)  # type: ignore
    comp.addTabPageContainerListener(source.events_listener_tab_page_container)

...
from ...common.props.cell_text_properties_props import CellTextPropertiesProps
...
class Properties(StyleBase):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if not wrap_auto is None:
            self.prop_wrap_auto = wrap_auto
        if not hyphen_active is None:
            self.prop_hyphen_active = hyphen_active
        if not shrink_to_fit is None:
            self.prop_shirnk_to_fit = shrink_to_fit
    ...
    @property
    def prop_shirnk_to_fit(self) -> bool | None:
        """
        Gets/Sets If text shinks to cell size.
        """
        return self._get(self._props.shrink)

    @prop_shirnk_to_fit.setter
    def prop_shirnk_to_fit(self, value: bool | None):
        if value is None:

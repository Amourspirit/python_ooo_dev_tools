import uno
from ooo.dyn.table.border_line_style import BorderLineStyleEnum as BorderLineStyleEnum
from ooo.dyn.table.shadow_format import ShadowFormat as ShadowFormat
from ooo.dyn.table.shadow_location import ShadowLocation as ShadowLocation
from ....style.char import StyleCharKind as StyleCharKind
from .....direct.structs.side import Side as Side, SideFlags as SideFlags, LineSize as LineSize
from .....modify.char.border.sides import Sides as Sides
from .....modify.char.border.padding import Padding as Padding
from .....modify.char.border.shadow import Shadow as Shadow

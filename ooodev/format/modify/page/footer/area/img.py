...
class Img(PageStyleBaseMulti):
    ...
    def from_style(
        ...
        direct = FooterImg.from_obj(inst.get_style_props(doc))
    ...
    def from_preset(
        ...
        inst = super(Img, cls).__new__(cls)
        inst.__init__(style_name=style_name, style_family=style_family)
        direct = FooterImg.from_preset(preset=preset)
        inst._set_style("direct", direct, *direct.get_attrs())
        return inst
    ...
    @property
    def prop_inner(self) -> FooterImg:
        """Gets Inner Image instance"""
        try:
            return self._direct_inner
        except AttributeError:
            self._direct_inner = cast(FooterImg, self._get_style_inst("direct"))
        return self._direct_inner

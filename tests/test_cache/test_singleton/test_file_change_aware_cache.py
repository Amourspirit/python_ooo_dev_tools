...
@pytest.fixture
def cache(loader):
    return FileChangeAwareCache(tmp_dir="test_cache", key="test_key")
...
def test_invalidate_if_changed(cache, tmp_path):
    data = {"name": "John", "age": 30, "city": "New York"}
    # Create a temporary file path
    temp_file = tmp_path / "temp_data.json"
...
def test_singleton(cache):
    cache2 = FileChangeAwareCache(tmp_dir="test_cache", key="test_key")
    assert cache is cache2
    cache3 = FileChangeAwareCache(tmp_dir="test_cache")

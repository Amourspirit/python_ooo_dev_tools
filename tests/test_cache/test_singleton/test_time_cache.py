...
@pytest.fixture
def cache():
    return TimeCache(key="test_key")


def test_singleton(cache):
    cache2 = TimeCache(key="test_key")
    ...
    cache5 = TimeCache(seconds=20)
    cache6 = TimeCache(seconds=20)
    assert cache5 is cache6

    cache7 = TimeCache(seconds=30, cleanup_interval=10)
    assert cache6 is not cache7

    cache8 = TimeCache(seconds=30, cleanup_interval=10)
    assert cache7 is cache8

    cache9 = TimeCache(key="test_key", key2="test_key2", key3=11)
    cache10 = TimeCache(key="test_key", key2="test_key2", key3=11)
    assert cache9 is cache10
...
def test_thread():
    ...
    cache = TimeCache(seconds=2, cleanup_interval=1)
    cache.put("key1", "value1")
    time.sleep(1)

...
def test_div_inch() -> None:
    u1 = UnitPT.from_inch(4)  # 5080 mm 100
    u2 = UnitInch(2)  # 5080 mm 100
    u3 = u1 / u2
    assert u3 == 2.0

    u1 = UnitPT.from_inch(10)  # 5080 mm 100
...
def test_plus_equals() -> None:
    u1 = UnitPT(2)
    u1 += 3
    assert isinstance(u1, UnitPT)
    assert u1 == 5
    u2 = UnitInch.from_pt(2)
    u1 += u2
    assert isinstance(u1, UnitPT)
    assert u1 == 7

...
from ooodev.units import UnitInch100
...
def test_sub_unit_inch() -> None:
    ...
    u3 = u1 - u2
    assert u3.almost_equal(324.0)  # 1.6 mm
...
def test_div_inch() -> None:
    u1 = UnitInch100.from_inch(4)  # 5080 mm 100
    u2 = UnitInch(2)  # 5080 mm 100
    u3 = u1 / u2
    assert u3 == 2.0

    u1 = UnitInch100.from_inch(10)  # 5080 mm 100
...
def test_greater_than_equal() -> None:
    u1 = UnitInch100(331.2)
    u2 = UnitInch100(330.0)
    assert u1 >= u2
    u2 = UnitInch100(331.2)
    assert u1 >= u2
...
def test_plus_equals() -> None:
    u1 = UnitInch100(2)
    u1 += 3
    assert isinstance(u1, UnitInch100)
    assert u1 == 5
    u2 = UnitInch.from_inch100(2)
    u1 += u2
    assert isinstance(u1, UnitInch100)
    assert u1 == 7

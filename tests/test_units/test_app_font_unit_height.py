...
def test_from_px(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        unit = UnitAppFontHeight.from_px(4)
...
def test_from_unit_val(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        px = UnitPX(10)
        unit = UnitAppFontHeight.from_unit_val(px)
        assert unit.value == 5.0

        assert unit.get_value_px() == 10.0

        unit = UnitAppFontHeight.from_unit_val(4)
...
def test_eq(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        ...
        assert unit == unit_px
        assert unit == 4.0
        assert unit == UnitAppFontHeight(4.0)
        assert unit != UnitAppFontHeight(4.2)
        assert unit != UnitAppFontWidth(unit.value)

        unit_mm = UnitMM.from_px(unit_px.value)
        assert unit == unit_mm

        u2 = UnitAppFontHeight.from_unit_val(unit)
        assert unit == u2

        u3 = UnitAppFontHeight.from_unit_val(unit.value)
        assert unit == u3
...
def test_lt(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        ...
        assert unit < unit_px
        assert unit < 4.1
        assert unit < UnitAppFontHeight(4.01)
...
def test_gt(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        ...
        assert unit > unit_px
        assert unit > 3.9
        assert unit > UnitAppFontHeight(3.99)
...
def test_lt_eq(loader) -> None:
    sp = GenericSizePos(x=0.6, y=0.5, width=0.6, height=0.5)
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        unit = UnitAppFontHeight(4.0)  # 8 px
        unit_px = UnitPX(8.05)
        assert unit <= unit_px
        assert unit <= 4.1
        assert unit <= UnitAppFontHeight(4.01)
...
def test_gt_eq(loader) -> None:
    ...
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        ...
        assert unit >= unit_px
        assert unit >= 4
        assert unit >= UnitAppFontHeight(4)
...
def test_add(loader) -> None:
    u1 = UnitAppFontHeight(1.55)
    u2 = UnitAppFontHeight(2.24)
    u3 = u1 + u2
...
def test_sub(loader) -> None:
    u1 = UnitAppFontHeight(3.77)
    u2 = UnitAppFontHeight(1.29)
    u3 = u1 - u2
    assert u3.almost_equal(2.48)


def test_sub_float(loader) -> None:
    u1 = UnitAppFontHeight(3.5)
    u3 = u1 - 1.5
...
def test_mul(loader) -> None:
    u1 = UnitAppFontHeight(3.7)
    u2 = UnitAppFontHeight(2.88)
    u3 = u1 * u2
    assert u3.almost_equal(10.656)


def test_mul_float(loader) -> None:
    u1 = UnitAppFontHeight(3.89)
    u3 = u1 * 2.87
...
def test_mul_px(loader) -> None:
    sp = GenericSizePos(x=0.6, y=0.5, width=0.6, height=0.5)
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        u1 = UnitAppFontHeight(4)  # 8 px, 4AF
        ...
        u1 = UnitAppFontHeight(4)  # 8px, 4AF
...
def test_div_px(loader) -> None:
    sp = GenericSizePos(x=0.6, y=0.5, width=0.6, height=0.5)
    with patch("ooodev.loader.lo.Lo._lo_inst._app_font_pixel_ratio", sp):
        u1 = UnitAppFontHeight.from_px(4)
        u2 = UnitPX(2)
        u3 = u1 / u2
        assert u3 == 2.0

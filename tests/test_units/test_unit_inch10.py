...
def test_sub_unit_inch() -> None:
    ...
    u3 = u1 - u2
    assert u3.almost_equal(504.0)  # 1.6 mm
...
def test_div_inch() -> None:
    u1 = UnitInch10.from_inch(4)  # 5080 mm 100
    u2 = UnitInch(2)  # 5080 mm 100
    u3 = u1 / u2
    assert u3 == 2.0

    u1 = UnitInch10.from_inch(10)  # 5080 mm 100
...
def test_plus_equals() -> None:
    u1 = UnitInch10(2)
    u1 += 3
    assert isinstance(u1, UnitInch10)
    assert u1 == 5
    u2 = UnitInch.from_inch10(2)
    u1 += u2
    assert isinstance(u1, UnitInch10)
    assert u1 == 7

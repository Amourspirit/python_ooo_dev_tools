...
def test_mul_inch() -> None:
    ...
    u2 = UnitInch(2)  # 5080 mm 100, 50.8 mm
    u3 = u1 * u2
    assert u3.almost_equal(12.9032)
...
def test_div_inch() -> None:
    u1 = UnitCM.from_inch(4)  # 5080 mm 100
    u2 = UnitInch(2)  # 5080 mm 100
    u3 = u1 / u2
    assert u3 == 2.0

    u1 = UnitCM.from_inch(10)  # 5080 mm 100
...
def test_plus_equals() -> None:
    u1 = UnitCM(2)
    u1 += 3
    assert isinstance(u1, UnitCM)
    assert u1 == 5
    u2 = UnitInch.from_cm(2)
    u1 += u2
    assert isinstance(u1, UnitCM)
    assert u1 == 7

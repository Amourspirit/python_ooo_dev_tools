...
def test_mul_inch() -> None:
    ...
    u2 = UnitInch(2)  # 5080 mm 100, 50.8 mm
    u3 = u1 * u2
    assert u3.almost_equal(487.68)


def test_mul_px() -> None:
    u1 = UnitPX(2.54)
    u2 = UnitMM.from_inch(2)  # 5080 mm 100, 50.8 mm
    u3 = u1 * u2
    assert u3.almost_equal(487.68)
...
def test_div_inch() -> None:
    u1 = UnitPX.from_inch(4)  # 5080 mm 100
    u2 = UnitInch(2)  # 5080 mm 100
    u3 = u1 / u2
    assert u3 == 2.0

    u1 = UnitPX.from_inch(10)  # 5080 mm 100
...
def test_plus_equals() -> None:
    u1 = UnitPX(2)
    u1 += 3
    assert isinstance(u1, UnitPX)
    assert u1 == 5
    u2 = UnitInch.from_px(2)
    u1 += u2
    assert isinstance(u1, UnitPX)
    assert u1 == 7

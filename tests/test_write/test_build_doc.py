...
def test_build_doc(loader,props_str_to_dict,fix_image_path, capsys: pytest.CaptureFixture):
    ...
    pos = append(text="Some examples of simple text ")
    assert pos == 29
    append("styles.")
    ...
    pos = para("Have a good breakfast")
    assert pos == 170
    para("Have a good lunch")
    para("Have a good dinner")
    nl()
    ...
    assert coord == '(22412, 9885)'
    
    ypos = tvc.getPosition().Y
    
    nl()
    pos = nl("public class Hello")
    nl("{")
    ...
    Write.style_prev_paragraph(
        cursor=cursor,
        prop_name="ParaAdjust",
        prop_val=Write.ParagraphAdjust.CENTER
    )

...
class AnimBicycle:
    ...
    def animate(self) -> None:
        ...
        if not FileIO.is_valid_path_or_str(self._fnm_bike):
            MsgBox.msgbox(
        ...
        fnm = FileIO.get_absolute_path(self._fnm_bike)
        if not fnm.exists():
            MsgBox.msgbox(f'File Not existing: "{self._fnm_bike}"', "File Error", boxtype=MessageBoxType.ERRORBOX)
            Lo.close_office()
            return
        if not fnm.is_file():
            MsgBox.msgbox(f'Path is not a file: "{self._fnm_bike}"', "File Error", boxtype=MessageBoxType.ERRORBOX)
            Lo.close_office()
            return

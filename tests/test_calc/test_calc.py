...
from ooodev.events.lo_events import is_meth_event
...
def test_create_doc_events(loader):
    ...
    def on(source: Any, args: CancelEventArgs) -> None:
        nonlocal doc_opening
        doc_opening = True
        assert args.event_data["loader"] is loader
        assert is_meth_event(source, Calc.create_doc)
        assert args.source is source
        assert args.event_name == CalcNamedEvent.DOC_CREATING
...
def test_get_sheet(loader) -> None:
    ...
    def on(source: Any, args: CancelEventArgs) -> None:
        nonlocal on_firing
        on_firing = True
        assert is_meth_event(source, Calc.get_sheet)
...
def test_insert_sheet(loader) -> None:
    ...
    def on(source: Any, args: CancelEventArgs) -> None:
        nonlocal on_firing
        on_firing = True
        assert is_meth_event(source, Calc.insert_sheet)
...
def test_remove_sheet(loader) -> None:
    ...
    def on(source: Any, args: SheetCancelArgs) -> None:
        nonlocal on_firing
        assert is_meth_event(source, Calc.remove_sheet)
        assert isinstance(args, SheetCancelArgs)
        on_firing = True

    def after(source: Any, args: SheetArgs) -> None:
        nonlocal on_fired
        assert is_meth_event(source, Calc.remove_sheet)
        assert isinstance(args, SheetArgs)
        on_fired = True
...
def test_move_sheet(loader) -> None:
    ...
    def on(source: Any, args: SheetCancelArgs) -> None:
        assert isinstance(args, SheetCancelArgs)
        nonlocal on_firing
        assert is_meth_event(source, Calc.move_sheet)
        on_firing = True
    ...
    def on_cancel(source: Any, args: SheetCancelArgs) -> None:
        args.cancel = True
    ...
    try:
        ...
        events.remove(CalcNamedEvent.SHEET_MOVING, on)
        events.on(CalcNamedEvent.SHEET_MOVING, on_cancel)

        assert Calc.move_sheet(doc, "myFailedSheet", 0) is False
        assert on_firing is False
        assert on_fired is False
...
def test_get_set_active_sheet(loader) -> None:
    ...
    def on(source: Any, args: SheetCancelArgs) -> None:
        nonlocal on_firing
        assert isinstance(args, SheetCancelArgs)
        assert is_meth_event(source, Calc.set_active_sheet)
        on_firing = True
...
def test_delete_row(loader) -> None:
    ...
    def on(source: Any, args: SheetCancelArgs) -> None:
        nonlocal on_firing
        assert isinstance(args, SheetCancelArgs)
        assert is_meth_event(source, Calc.delete_row)
        on_firing = True
...
def test_insert_column(loader) -> None:
    ...
    def on(source: Any, args: SheetCancelArgs) -> None:
        nonlocal on_firing
        assert isinstance(args, SheetCancelArgs)
        assert is_meth_event(source, Calc.insert_column)
        assert args.index == 1
        on_firing = True
    ...
    try:
        ...
        val = Calc.get_val(sheet=sheet, cell_name="D2")
        # Lo.delay(1500)
        assert val == "hello world"
    finally:
        Lo.close(closeable=doc, deliver_ownership=False)
...
def test_insert_cells_down(loader) -> None:
    ...
    def on(source: Any, args: CellCancelArgs) -> None:
        ...
        assert is_meth_event(source, Calc.insert_cells)
...
def test_delete_cells_down(loader) -> None:
    ...
    def on(source: Any, args: CellCancelArgs) -> None:
        nonlocal on_firing
        assert isinstance(args, CellCancelArgs)
        assert is_meth_event(source, Calc.delete_cells)
    ...
    try:
        ...
        assert Calc.delete_cells(sheet=sheet, cell_range=rng, is_shift_left=False)
        assert on_firing
        assert on_fired
        on_firing = False
        on_fired = False

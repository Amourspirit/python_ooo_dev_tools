...
def do_cell_ranges() -> None:
    ...
    rng_str  = insert_range(src_con, 0, 0, 0, 0, False);  # A1:A1  -- empty
    assert rng_str == 'Sheet1.A1'
    rng_str = insert_range(src_con, 0, 1, 0, 2, True);   # A2:A3  -- empty
    assert rng_str == 'Sheet1.A1:A3'
    rng_str = insert_range(src_con, 1, 0, 1, 2, False);  # B1:B3   -- empty
    assert rng_str == 'Sheet1.A1:A3,Sheet1.B1:B3'

    rng_str = insert_range(src_con, 0, 9, 1, 29, False); # A10:B30  -- not empty
...
def insert_range(src_con: XSheetCellRangeContainer, start_col:int, start_row: int, end_col: int, end_row: int, is_merged: bool) -> str:

...
def test_replace_all(loader) -> None:
    ...
    try:
        ...
        cell_range = Calc.get_cell_range(
            sheet=sheet, start_col=0, start_row=0, end_col=total_cols - 1, end_row=total_rows - 1
        )  # A1:F15
...
def do_search(
    ...
    add_label(
        doc=doc, sheet=sheet, empty_row_num=total_rows + 1, col_span=total_cols - 1, text=f"Searching for {search_wrd}"
    )
    ...
    add_label(
        doc=doc,
        sheet=sheet,
        empty_row_num=total_rows + 1,
        col_span=total_cols - 1,
        text=f"Replacing {search_wrd} with {repl_str}",
    )
...
def add_label(doc: XSpreadsheetDocument, sheet: XSpreadsheet, empty_row_num: int, col_span: int, text: str) -> None:
    ...
    cell_range = Calc.get_cell_range(
        sheet=sheet, start_col=0, start_row=empty_row_num, end_col=col_span, end_row=empty_row_num
    )

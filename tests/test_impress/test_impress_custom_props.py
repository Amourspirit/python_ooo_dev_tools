...
def test_impress_custom_props(loader, tmp_path) -> None:
    # get_sheet is overload method.
    # testing each overload.
    doc = None
    try:
        pth = Path(tmp_path, "test_impress_custom_props.odp")
        ...
        assert dd.boolean is True
        assert len(dd) == 3
        assert hasattr(dd, "doc_name")

        page2.remove_custom_property("doc_name")
        dd = page2.get_custom_properties()
        assert hasattr(dd, "doc_name") is False
...
def test_doc_custom_props(loader, tmp_path) -> None:
    ...
    doc = None
    try:
        pth = Path(tmp_path, "test_doc_custom_props.odp")
        doc = ImpressDoc.create_doc(loader)
        dd = DotDict()
        dd.test1 = 10
